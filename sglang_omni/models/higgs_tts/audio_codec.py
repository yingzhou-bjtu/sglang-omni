...
class HiggsAudioCodec:
    ...
    def capture_decode_cuda_graphs_locked(self, frame_counts: tuple[int, ...]) -> None:
        ...
        # Stream, pool and synchronize helpers come from the device module
        # (`torch.get_device_module` is what the other capturing models use);
        # graph capture itself goes through the platform backend, which already
        # knows that MUSA presents through torch.cuda.
        device_module = torch.get_device_module(self.device)
        graph_backend = current_platform.get_device_graph_backend(self.device)
        if graph_backend is None:
            raise RuntimeError(
                f"Higgs codec graphs are not available on {self.device.type}"
            )
